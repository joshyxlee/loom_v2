import 'shop_models.dart';

const mistakeShieldItemId = 'boost_mistake_shield';

List<ShopItem> buildShopCatalog() {
  return const [
    ShopItem(
      id: mistakeShieldItemId,
      titleZh: '失誤保護卡',
      subtitleZh: '第一次答錯不扣分，保住這一回合。',
      category: ShopCategory.boost,
      priceTokens: 150,
      kind: ShopItemKind.consumable,
      isEnabled: true,
      effect: ShopEffect.mistakeShield,
    ),
    ShopItem(
      id: 'boost_focus_xp',
      titleZh: '專注強化',
      subtitleZh: '+20% XP（接下來 5 題）',
      category: ShopCategory.boost,
      priceTokens: 120,
      kind: ShopItemKind.consumable,
      isEnabled: true,
      effect: ShopEffect.focusXp,
    ),
    ShopItem(
      id: 'boost_double_token',
      titleZh: '雙倍獎勵',
      subtitleZh: '接下來 3 次答對，知識幣×2',
      category: ShopCategory.boost,
      priceTokens: 180,
      kind: ShopItemKind.consumable,
      isEnabled: true,
      effect: ShopEffect.doubleToken,
    ),
    ShopItem(
      id: 'boost_xp_burst',
      titleZh: '爆發加成',
      subtitleZh: '+50% XP（接下來 3 題）',
      category: ShopCategory.boost,
      priceTokens: 160,
      kind: ShopItemKind.consumable,
      isEnabled: true,
    ),
    ShopItem(
      id: 'boost_streak_saver',
      titleZh: '連勝保護卡',
      subtitleZh: '錯過一天也能保住連勝',
      category: ShopCategory.boost,
      priceTokens: 200,
      kind: ShopItemKind.consumable,
      isEnabled: true,
    ),
    ShopItem(
      id: 'util_skip_question',
      titleZh: '跳題券',
      subtitleZh: '跳過這題，不扣分、不計次',
      category: ShopCategory.utility,
      priceTokens: 80,
      kind: ShopItemKind.consumable,
      isEnabled: true,
    ),
    ShopItem(
      id: 'util_hint_reveal',
      titleZh: '提示券',
      subtitleZh: '排除兩個錯誤選項',
      category: ShopCategory.utility,
      priceTokens: 90,
      kind: ShopItemKind.consumable,
      isEnabled: true,
    ),
    ShopItem(
      id: 'util_reroll_question',
      titleZh: '換題券',
      subtitleZh: '換一題新的（不計次）',
      category: ShopCategory.utility,
      priceTokens: 60,
      kind: ShopItemKind.consumable,
      isEnabled: true,
    ),
    ShopItem(
      id: 'cosmetic_theme_night',
      titleZh: '夜間主題',
      subtitleZh: '更舒服的暗色介面',
      category: ShopCategory.cosmetic,
      priceTokens: 200,
      kind: ShopItemKind.equipable,
      isEnabled: true,
    ),
    ShopItem(
      id: 'cosmetic_theme_ocean',
      titleZh: '海洋主題',
      subtitleZh: '清爽的藍色調',
      category: ShopCategory.cosmetic,
      priceTokens: 200,
      kind: ShopItemKind.equipable,
      isEnabled: true,
    ),
    ShopItem(
      id: 'cosmetic_theme_warm',
      titleZh: '暖陽主題',
      subtitleZh: '溫暖柔和的色調',
      category: ShopCategory.cosmetic,
      priceTokens: 200,
      kind: ShopItemKind.equipable,
      isEnabled: true,
    ),
    ShopItem(
      id: 'pack_ai',
      titleZh: 'AI 世代',
      subtitleZh: '關於人工智慧、科技巨頭與未來想像。從 ChatGPT 到科技革命，你準備好了嗎？',
      category: ShopCategory.unlock,
      priceTokens: 200,
      kind: ShopItemKind.owned,
      isEnabled: true,
    ),
    ShopItem(
      id: 'pack_kpop',
      titleZh: '韓流現象',
      subtitleZh: 'K-Pop、韓劇與娛樂產業背後的故事。流行之外，你知道多少？',
      category: ShopCategory.unlock,
      priceTokens: 200,
      kind: ShopItemKind.owned,
      isEnabled: true,
    ),
    ShopItem(
      id: 'pack_nba',
      titleZh: 'NBA 焦點',
      subtitleZh: '球星、紀錄與經典時刻。每一次得分，都有故事。',
      category: ShopCategory.unlock,
      priceTokens: 200,
      kind: ShopItemKind.owned,
      isEnabled: true,
    ),
    ShopItem(
      id: 'pack_business',
      titleZh: '商業與品牌',
      subtitleZh: '品牌為什麼成功？決策如何改變世界？商業，比你想像的更有趣。',
      category: ShopCategory.unlock,
      priceTokens: 200,
      kind: ShopItemKind.owned,
      isEnabled: true,
    ),
    ShopItem(
      id: 'limited_bundle_starter',
      titleZh: '新手補給包',
      subtitleZh: '今日限時組合',
      category: ShopCategory.limited,
      priceTokens: 250,
      kind: ShopItemKind.owned,
      isEnabled: true,
    ),
    ShopItem(
      id: 'limited_bundle_booster',
      titleZh: '爆發加速包',
      subtitleZh: '今日限時組合',
      category: ShopCategory.limited,
      priceTokens: 450,
      kind: ShopItemKind.owned,
      isEnabled: true,
    ),
  ];
}
